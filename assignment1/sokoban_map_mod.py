...
class SokobanMapMod:
    ...
    def apply_move(self, move, is_simulation):
        """
        ...
        if (new_y, new_x) in self.box_positions:
            if move == self.LEFT:
                ...
                if self.obstacle_map[new_y][new_x - 1] == self.OBSTACLE_SYMBOL or (new_y, new_x - 1) in self.box_positions:
                    ...
                    if new_x - 2 == 0:  # positioned by the left obstacle after box has been pushed
                        # target is down while up has no space
                        if new_y - 1 >= 1 and self.obstacle_map[new_y - 1][new_x - 1] == self.OBSTACLE_SYMBOL:
                            return None
                        # target is up while down has no space
                        elif new_y + 1 <= self.y_size and self.obstacle_map[new_y + 1][new_x - 1] == self.OBSTACLE_SYMBOL:
                            return None
                        # target is right
                        elif new_y + 1 <= self.y_size and self.obstacle_map[new_y + 1][new_x - 1] == self.OBSTACLE_SYMBOL:
                            return None
    ...
    def __eq__(self, other):
        if not isinstance(other, SokobanMapMod):
            # don't attempt to compare against unrelated types
            return NotImplemented

        return self.box_positions == other.box_positions
